...
const Generatefront = ({ nameStr, nationStr, addrStr, idStr, headImgUrl }) => {
  const [headImageSrc, setHeadImageSrc] = d("");
  const [cellSize] = d(6);
  ...
  y(() => {
    if (headImgUrl) {
      ...
      const gender = getGenderFromID(idStr);
      setHeadImageSrc(gender === "男" ? `${baseURL.replace(/\/$/, "")}/id_img_head_man.webp` : `${baseURL.replace(/\/$/, "")}/id_img_head_woman.webp`);
    }
  }, [headImgUrl, idStr, baseURL]);
  y(() => {
...
const App = () => {
  ...
  const fillRandomInfo = async () => {
    ...
    setNationVal(data.nation);
    setIdVal(data.idNumber);
    setAddrVal(data.address);
    setIssueVal(data.issuingAuthority);
    setStartDate(data.issueStart);
    setEndDate(data.issueEnd);
  };
  ...
  return /* @__PURE__ */ u$1("div", { children: [
          ...
          /* @__PURE__ */ u$1(
            "button",
            {
              ...
              onClick: () => {
                ...
                const filename = isShowingBack ? "身份证背面.jpg" : "身份证正面.jpg";
                if (!currentImageUrl) {
                  alert("当前图像未生成");
                  return;
                }
    ...
    /* @__PURE__ */ u$1("div", { className: "input-list", children: [
      /* @__PURE__ */ u$1("div", { className: "input-horizon", children: /* @__PURE__ */ u$1("div", { className: "input-box-upload-vertical", children: [
